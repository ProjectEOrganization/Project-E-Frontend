  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (newFriendRef == "true") {
        ...
        await AsyncStorage.setItem("newFriendRef", "true").catch((error) =>
          console.log(error)
        );
      }
    }
  ...
  return (
    <View style={[styles.topBar, { paddingTop: top + 10 }]}>
      ...
      <View style={styles.userNameText}>
        ...
        <Text numberOfLines={1} adjustsFontSizeToFit style={styles.firstText}>
          {user?.isFriends ? user?.displayName : user?.secretDisplayName}
        </Text>
        <Text style={[{ color: user?.isActive == true ? "green" : "red" }]}>
          {user?.isActive == 1 ? "Active" : "Offline"}
        </Text>
      </View>
      ...
      <TouchableOpacity
        onPress={() => navigation.navigate("ReportBlockModal")}
        style={{
