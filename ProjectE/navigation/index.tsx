...
import Onboarding3 from "../screens/Onboarding4";
import Onboarding4 from "../screens/Onboarding5";
      ...
      async (response) => {
        if (response.notification.request.content.data.type === "dm") {
          const chat = await store.dispatch(
            loadChat(response.notification.request.content.data.uid)
          );
          navigationRef.current?.navigate("FriendsChatScreen", chat.payload);
        } else if (
          response.notification.request.content.data.type === "friend_request"
        ) {
          navigationRef.current?.navigate("FriendRequestReceivedModal", {
            uid: response.notification.request.content.data.uid,
          });
        }
      }
    );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Stack.Screen name="FindingScreen" component={FindingScreen} />
        <Stack.Screen
          name="NotFound"
